...
async def main(
    config_dir: str = "module_validator/config",
    ...
    return (
        base64.b85decode(result.decode("utf-8"))
...
if __name__ == "__main__":
    from module_validator.modules.translation.data_models import (
        TARGET_LANGUAGES,
        TASK_STRINGS,
    )

    data_map = {"1": "translation", "2": "embedding", "3": "financialnews"}
    task_map = {
        "1": "text2text",
        "2": "text2speech",
        "3": "speech2text",
    ...
    result = asyncio.run(
        main(
            config_dir="module_validator/config",
            module_name=data_map[choice],
            input_data=data,
        )
    )
    logger.debug(result)
