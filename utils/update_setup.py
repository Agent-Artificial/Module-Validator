            ...
            for keyword in node.keywords:
                if keyword.arg == 'entry_points':
                    ...
                    for key in keyword.value.keys:
                        if isinstance(key, ast.Str) and key.s == 'module_validator.inference':
                            ...
                            if isinstance(key.value, ast.List):
                                ...
                                key.value = ast.List(elts=[
                                    ast.Str(s=existing_entry),
                                    ast.Str(s=new_entry)
                                ], ctx=ast.Load())
                            else:
                                print(f"Unexpected type for module_validator.inference value: {type(key.value)}")
                                return
